...
export async function GET(
  ...
  const { id } = params;
  try {
    const data = await prisma.thuocTinh.findUnique({
      where: {
        id: Number(id),
      },
    });
    if (!data) {
      return NextResponse.json(
        { message: "Không có thuộc tính nào có id này" },
        { status: 400 }
      );
    }
...
export async function PUT(
  ...
  try {
    const newThuocTinh = await prisma.thuocTinh.updateMany({
      where: {
        id: Number(id),
      },
      data: {
...
export async function DELETE(
  ...
  const { id } = params;
  try {
    const deleted = await prisma.thuocTinh.delete({
      where: {
        id: Number(id),
      },
    });
