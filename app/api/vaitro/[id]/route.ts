...
export async function GET(
  ...
  const { id } = params;
  try {
    const data = await prisma.vaiTro.findUnique({
      where: {
        id: Number(id),
      },
    });
...
export async function PUT(
  ...
  const successfull = VaiTroSchema.safeParse(data);
  if (!successfull.success) {
    return NextResponse.json({ error: successfull.error }, { status: 400 });
  }

  try {
    const newNhanVien = await prisma.vaiTro.updateMany({
      where: {
        id: Number(id),
      },
      data: {
        tenVaiTro: successfull.data.tenVaiTro,
      },
    });
...
export async function DELETE(
  ...
  const { id } = params;
  try {
    const deleted = await prisma.vaiTro.delete({
      where: {
        id: Number(id),
      },
    });
